            ...
            for k, v in col_name_dict.items():
                ...
                for k, v in col_name_dict.items():
                    if year_start==1998:
                        ...
                        future_Data2 = pd.read_excel('../data/future_2_Y9_2_1998_1123.xlsx', sheetname='Sheet1')
                    elif industry_labels==[]:
                        otherFeatureList = [9, 13, 16]  ###删掉12,13,12在后面有，13为标签
                        baseFeatureList = [39, 57, 93, 149, 207, 261, 279, 297, 315, 333, 351,387]
                        future_Data = pd.read_excel('../data/future_2_Y9_2_!1998_all_1114.xlsx', sheetname='Sheet1')
                        future_Data2 = pd.read_excel('../data/future_2_Y9_2_!1998_all_1123.xlsx', sheetname='Sheet1')
                        ...
                        future_Data2 = pd.read_excel('../data/future_2_Y9_2_!1998_label2_1123.xlsx', sheetname='Sheet1')
                    # baseFeatureList = [369]
                    for i in range(len(otherFeatureList)):  ###
                        otherFeatureList[i] -= 1
                    for i in range(len(baseFeatureList)):  ###
...
#############################################################################################################
                ###HW上验证
                print 'year:%d-%d\t' % (year_start, year_end) + 'industry_label:%s' % str(industry_labels)
    ...
    ##############################################################################################################3
                    ...
                    datalist = future_Data2.iloc[:, :].values
                    pred_future = model.predict(np.array(datalist))
                    for num in range(len(pred_future)):
                        print str(pred_future[num]) + '\t',
                    print
