...
# for i in range(total_colnum,total_colnum+1):
#     slide_pred={'year_start':'201509','year_end':'201609','size':1}
...
#
#     year_last= ((int(slide_pred['year_end']) / 100 - int(slide_pred['year_start']) / 100) * 12 + int(slide_pred['year_end']) % 100-int(slide_pred['year_start']) % 100+1)
...
#     plt.plot(pred_data[:len(month_list)], color='red')
#     plt.plot(real_data, color='black')
#     plt.savefig('../result/0927' + u'合并抵消' + '/' + var_name + '_' + str(train_year) + '_' + slide_pred['year_start']+'_'+slide_pred['year_end'] +'_size_'+str(slide_pred['size'])  + '_contrast.png')
#     for j in range(len(real_data)):
#         print str(month_list[j]) + ':\t' + str(pred_data[j]) + '\t' + str(real_data[j])
#     print
#     # print 'the next %d month:' % month_num
