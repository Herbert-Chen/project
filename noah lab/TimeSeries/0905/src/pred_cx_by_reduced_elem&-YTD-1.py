...
elem = ['AP', 'AQ', 'AR', 'AT', 'AU', 'AV', 'AW', 'AY', 'AZ', 'BA', 'BB', 'BC', 'BD', 'BX', 'BY', 'CA', 'CD', 'CE',
        'CF', 'CG', 'CH', 'CO', 'CN', 'CT', 'CV', 'CX']
algo = 'gbrt'
...
for i in range(1, 193):
    ...
    reduced_array = np.column_stack((reduced_array_i_13[:, :3], reduced_array_i_14[:, 2], reduced_array_i_15[:, 2],
                                     reduced_array_i_16[:, 2], reduced_array_i_13[:, 3:15], reduced_array_i_14[:, 3:15],
                                     reduced_array_i_15[:, 3:15]))
    train_data = []
    train_label = []
    for j in range(23):
        temp = list(reduced_array[:, 6 + j] - reduced_array[:, 3 + j / 12])
    ...
    test_data = list(reduced_array[:, 6 + 23] - reduced_array[:, 4])

    for k in range(12):
        pred_cx = model.predict([test_data])
        print str(pred_cx[0]+reduced_array[-1, 5]) + '\t',
        test_data = list(reduced_array[:, 6 + 23 + 1 + k]-reduced_array[:, 5])
        test_data[-1] = pred_cx
