...
for i in range(1, 193):
    ...
    train_label = []
    for j in range(23):
        temp = list(reduced_array[:, 6 + j] - reduced_array[:, 2 + j / 12])
    ...
    test_data = list(reduced_array[:, 6 + 23] - reduced_array[:, 3])

    for k in range(12):
        pred_cx = model.predict([test_data])
        print str(pred_cx[0]+reduced_array[-1, 4]) + '\t',
        test_data = list(reduced_array[:, 6 + 23 + 1 + k]-reduced_array[:, 4])
        test_data[-1] = pred_cx
