...
if __name__ == '__main__':
    ...
    ###所有变量循环
            for k, v in col_name_dict.items():
                ...
                test_time_series_log=[log(test_time_series[num]) for num in range(len(test_time_series))]
                ...
                diff = pd.DataFrame(test_time_series_log).diff().dropna()
                ...
                for num in range(model.predict(0).shape[0]):
...
#####
                pred_log=[test_time_series_log[0]]
                for num in range(len(test_time_series_log)):
                    pred_log.append(pred_1[num]+float(test_time_series_log[num]))
                for num in range(pred_year-1):
                    pred_log.append(pred_1[num+len(test_time_series_log)] + pred_log[num+len(test_time_series_log)])

                pred=[exp(pred_log[num]) for num in range(len(pred_log))]

                test=adfuller(diff_1.T.values[0],1)
                print '平稳检验p值：'+str(test[1])
                print(u'BIC最小的p值和q值为：%s、%s' % (p, q))
                ...
                plt.xlabel('year')
                plt.savefig('../result/1012/time_series_' + var_name + '_' +  '_contrast.png')
                plt.legend()
                plt.close()
