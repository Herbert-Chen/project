        ...
        for j in range(1,len(df1.columns)-train_year):
            ...
            for k in range(train_year+1):               ##训练和标签都不包含预测月
                if str(df1.columns[j+k]) in predict_month:
                    ...
                    # continue
            if flag == True:
                temp=list(df2.iloc[i,j:(j+train_year)].values)
                temp.extend([df1.iloc[i, j + train_year]])
...
for var_name in ['B']:
    ...
    for predict_month in ['201412','201506','201512','201606']:
        for train_year in [1,3,6]:
            ...
            train_and_label_data=transform(oriData3,oriData4)
        ###对每个公司：
            for i in range(total_colnum,total_colnum+1):
                ...
                temp=list(oriData4.iloc[i,colnum-train_year:colnum].values)
    ...
    for train_year in [1, 3, 6]:
        ...
        for i in range(total_colnum, total_colnum+1):
            ...
            test_data1=[]
            temp = list(oriData4.iloc[i,  32-train_year:32].values)
            test_data1.extend([temp])
            ###201608的A3数据
            pred_data1 = model.predict(test_data1)
            ...
            test_data=list(oriData4.iloc[i,32-train_year+1:32])
