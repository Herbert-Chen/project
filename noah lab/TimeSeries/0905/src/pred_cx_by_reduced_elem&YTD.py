...
for i in range(1,193):
    ...
    reduced_array=np.column_stack((reduced_array_i_13[:,:3],reduced_array_i_14[:,2],reduced_array_i_15[:,2],reduced_array_i_13[:,3:15],reduced_array_i_14[:,3:15],reduced_array_i_15[:,3:15]))
    train_data=[]
    train_label=[]
    for j in range(23):
        temp=list(reduced_array[:,5+j])
        temp.extend(list(reduced_array[:,2+j/12]))
        train_data.append(temp)
        train_label.append(reduced_array[-1,5+j+1])
    ...
    for k in range(12):
        print str(reduced_array[ - 1, 29+k])+'\t',
    print '\t\t',

    test_data=list(reduced_array[:,5+23])
    test_data.extend(reduced_array[:,3])
    for k in range(12):
        pred_cx=model.predict([test_data])
        print str(pred_cx[0])+'\t',
        test_data=list(reduced_array[:,5+23+1+k])
        test_data[-1]=pred_cx
        test_data.extend(reduced_array[:, 3])
