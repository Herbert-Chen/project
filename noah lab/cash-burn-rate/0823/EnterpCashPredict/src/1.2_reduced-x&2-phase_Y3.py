...
if __name__ == '__main__':
    ...
    for k,v in col_name_dict.items():
        ...
        otherFeatureList = [23, 24]     # unique features   ###新增cash flow risk，25列,删除12列(11),改为23
        ...
        label_name = oriData.columns[labelCol].replace('[FY %s]'%str(1997+year_gap+1), '')  ###
        data_set_no_zero = data_set[data_set[:, -1] != 0.0]
        feature_set, label_set = split_feature_label(data_set_no_zero)

        plt = ArkPlot()
        plt_out_name = '../result/1.2/' + 'cv_Y3' + algo + '_gap_' + str(year_gap) + '_last_' + str(year_last) + '.png'  ###
        ...
        # logger_file_name = '../result/1.2/cv_Y3' + algo + '_gap_' + str(year_gap) + '_last_' + str(year_last) + '.txt'
        ...
        ###
        plt_hw = ArkPlot()
        plt_hw_out_name = '../result/1.2/' + 'testone_Y3' + '_' + algo + '_gap_' + str(year_gap) + '_last_' + str(
            year_last) + '.png'

        print train_data_set.shape

        logger_file_name = '../result/1.2/testone_Y3_' + algo + '_gap_' + str(year_gap) + '_last_' + str(
            year_last) + '.txt'
        logger = open(logger_file_name, 'w')
        min_error = []
        mean_error = []
        ...
        pyplot.title(k)
        pyplot.xlabel('year')
        pyplot.savefig('../result/1.2/' + 'contrast' + '_Y3_' + algo + '_gap_' + str(year_gap) + '_last_' + str(
            year_last) + '.png')
        pyplot.legend()
        pyplot.close()
