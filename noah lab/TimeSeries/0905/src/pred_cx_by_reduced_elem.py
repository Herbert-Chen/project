...
for i in range(1,193):
    ...
    reduced_array=np.column_stack((reduced_array_i_13,reduced_array_i_14[:,3:15],reduced_array_i_15[:,3:15]))
    train_data=[]
    train_label=[]
    for j in range(23):
        train_data.append(reduced_array[:,3+j])
        train_label.append(reduced_array[-1,3+j+1])
    ...
    for k in range(12):
        print str(reduced_array[ - 1, 27+k])+'\t',
    print '\t\t',

    test_data=[reduced_array[:,3+23]]
    for k in range(12):
        pred_cx=model.predict(test_data)
        print str(pred_cx[0])+'\t',
        test_data=[reduced_array[:,3+23+1+k]]
        test_data[0][-1]=pred_cx
