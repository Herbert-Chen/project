...
def get_cleaned_dataset(df_train, df_test,colsList, industry_label,dev_label):
    ...
    # if dev_label !=[]:
    #     # df_train = df_train[ df_train['Industry Dummy'] in industry_label]
    #     df_train = df_train[df_train.iloc['Dev_label'] == dev_label]       ###扩展！！！！！
    # else:
    #     pass
    col_names = df_train.columns[colsList[0]]
    count = 0
    ...
    for indexes in colsList:
        ...
        testTmpArray = np.array(df_test.iloc[:, indexes[:-2]+[indexes[-1]]].values)
        if count == 0:
    ...
    for indexes in colsList:
        if dev_label != []:
            for companyrow in range(118):
                if df_train.iloc[companyrow,indexes[-2]] ==dev_label:
                    tmpArray = np.array(df_train.iloc[companyrow, indexes[:-2]+[indexes[-1]]].values)
                    if count == 0:
                        upperArray = tmpArray
                        count += 1
                        # continue
            ...
            tmpArray = np.array(df_train.iloc[:,indexes[:-2]+[indexes[-1]]].values)
            if count == 0:
                upperArray = tmpArray
                count += 1
                # continue
...
otherFeatureList = [12, 22, 23, 25]  # unique features
# position that years features first occur
baseFeatureList = [36, 56, 76, 96, 116, 136, 156, 176, 196, 216, 236, 256, 276,
                   296, 316, 336, 356, 376, 394, 416, 436, 476, 496, 516, 536,
                   556, 596, 616, 636, 656, 796, 816, 836, 856, 876, 896, 916]
    ...
    for var_name in ['Y2','Y3','Y5','Y6','Y7']:
        ...
        for year_start,year_end in [(1998,2015),(2010,2015),(1999,2002),(2007,2010)]:
            if var_name == 'Y2':
                ...
                # otherFeatureList = [12, 22, 23, 25, 993]  # unique features
                # # position that years features first occur
                # baseFeatureList = [36, 56, 76, 96, 116, 136, 156, 176, 196, 216, 236, 256, 276,
                #                    296, 316, 336, 356, 376, 394, 416, 436, 476, 496, 516, 536,
                #                    556, 596, 616, 636, 656, 796, 816, 836, 856, 876, 896, 916]

                otherFeatureList = [12,22, 25]
                baseFeatureList = [36,56,76, 96, 116, 136, 156, 236, 256, 276, 296, 316, 336, 356, 416, 436, 496, 516, 556, 816, 896, 916, 936,976]
            elif var_name == 'Y3':
                # 削减变量Y3
                col_name_dict = {'Total Cash & ST Investments/Avg. monthly cash outflow Month': 676}
                # otherFeatureList = [12, 25,993]
                ...
                #                    556, 596, 616, 636, 656, 796, 816, 836, 856, 876, 896, 916]
                #去掉12
                otherFeatureList = [22, 25]
                baseFeatureList = [56,76,96,116,136,156,236,256,276,296,316,336,356,416,436,496,516,556,876,896,916,936,976	]
            elif var_name == 'Y5':
                # 削减变量Y5
                col_name_dict = {'Total Cash & ST Investments/Total Assets %': 736}
                ...
                #去掉896
                otherFeatureList = [12,22, 25]
                baseFeatureList = [36,56,76,96,116,136,156,236,256,276,296,336,356,416,436,496,516,556,816,856,876,916,936,976	]
            elif var_name == 'Y7':
                # 削减变量Y7
                col_name_dict = {'Net Cash 1 / Sales %': 776}
                # otherFeatureList = [12, 25,993]
                ...
                otherFeatureList = [12,22, 25]
                baseFeatureList = [56,76,96,116,136,156,236,256,276,296,316,336,356,416,436,496,516,556,816,876,896,916,936,976]
            elif var_name == 'Y1':
                # 削减变量Y5
        ...
        ###所有变量循环
                for dev_label in [[]]:
                    ...
                    for k, v in col_name_dict.items():
                        ...
                        # print
                        print '1107预测未来：17-21' + '\t\t\t\t\t\t' + '1122预测未来：17-21'
                        for tt in range(10):
                            ###HW上验证
                            ...
                            # print var_name+'_industrylabel_'+str(industry_labels)+'_devlabel_'+str(dev_label)+'_'+str(year_start)+'-'+str(year_end)
                            # print '1107预测未来：17-21第%d次'%(tt+1)+'\t\t\t\t\t\t'+'1122预测未来：17-21第%d次'%(tt+1)
